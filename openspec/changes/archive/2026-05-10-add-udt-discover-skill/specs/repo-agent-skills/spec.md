...
### Requirement: UDT discovery skill resolves platform discovery for web use

The repository SHALL provide a local skill at `.codex/skills/udt-discover/SKILL.md`.

The skill SHALL trigger for user requests such as `udt:discover`, `udt-discover`, and requests to resolve `act/discover-platforms.md` for web use.

The skill SHALL resolve `act/discover-platforms.md` by inlining each required contract under a heading naming the source file, appending the prompt body, and producing one copy-ready prompt block.
...
The skill SHALL tell the researcher to save the web response to `observe/platform-discovery-<model-short>.md`.

#### Scenario: Researcher invokes udt discovery

- **WHEN** a researcher asks for `udt:discover`
- **THEN** the agent resolves `act/discover-platforms.md` for web use
