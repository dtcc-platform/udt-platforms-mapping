...
The benchmark report SHALL include a recall table with entity, expected type, tags, and one result column per tested model.

The benchmark report SHALL include novel finds grouped by model, using the columns `Name`, `Link`, `Type`, `Aliases`, and `Tags`.
