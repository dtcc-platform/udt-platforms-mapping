...
Canonical web prompt templates that conform to this contract and ask models to emit Markdown SHALL conform to `repo-prompt-markdown-format`.

The web prompt template contract SHALL NOT duplicate the full shared Markdown formatting contract.

#### Scenario: Prompt emits Markdown

- **WHEN** a conforming canonical web prompt asks a model to emit Markdown
- **THEN** it uses the shared Markdown formatting contract
