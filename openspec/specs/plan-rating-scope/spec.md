# Spec: plan-rating-scope

## Purpose

Defines the requirements for `plan/rating/scope.md`, the scope reference file for rating sessions. Researchers paste its full content into the `[PASTE_SCOPE_HERE]` slot in the rating prompt before running a session.

## Requirements

### Requirement: plan/rating/scope.md exists and contains the 12 dimension rubrics

The repository SHALL contain a file at `plan/rating/scope.md`. This file is the sole scope reference for rating sessions — researchers paste its full content into the `[PASTE_SCOPE_HERE]` slot in the rating prompt before running a session.
...
- **THEN** they paste the full content of `plan/rating/scope.md` into the `[PASTE_SCOPE_HERE]` slot in the rating prompt — not the discovery scope
...
- **THEN** they edit `plan/rating/scope.md` only; the rating prompt receives the updated rubric at run time via the paste step

#### Scenario: Researcher runs discovery after updating rating scope

- **WHEN** a researcher updates `plan/rating/scope.md`
