...
- A pipe table with columns: `Name`, `Link`, `Expected Layer`, `Why tricky`
...
The file SHALL be maintained by researchers: new entries are added when a known platform is found to be missing from a discovery response; new gap categories are added when a new class of discovery failure is identified.
...
- **THEN** it reads `tests/discovery-fixtures.md` and extracts all expected platforms with their gap category and expected Layer
