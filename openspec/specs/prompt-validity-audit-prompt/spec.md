...
Defines the CLI maintenance prompt at `tools/prompt-validity/prompt.md` that audits live repository prompts for validity and freshness against their governing specs and related files.

## Requirements

### Requirement: Prompt-validity audit prompt exists as a CLI maintenance prompt

The repository SHALL contain a file at `tools/prompt-validity/prompt.md`. This file SHALL be a CLI-only maintenance prompt that audits live repository prompt files for validity and freshness. It SHALL NOT be a Web-mode prompt.
...
- **WHEN** a researcher tells their AI CLI to run `tools/prompt-validity/prompt.md`
...
The audit prompt SHALL write its results to `tools/prompt-validity/report.md`, overwriting any existing file.

The prompt SHALL be runnable by a researcher telling the AI CLI either `run tools/prompt-validity/prompt.md` or `run the prompt validity audit`.

#### Scenario: Researcher reruns the audit after a change
- **WHEN** a researcher reruns the prompt-validity audit
- **THEN** the previous `tools/prompt-validity/report.md` is replaced with a new report reflecting the current repository state
