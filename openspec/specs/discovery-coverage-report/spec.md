...
Defines the structure and naming conventions for coverage reports generated by the discovery eval prompt. Reports are saved to `evals/discovery/reports/` and enable tracking of recall performance across models and over time.

## Requirements

### Requirement: Coverage report is structured in two parts: per-gap-category table and summary

The coverage report saved to `evals/discovery/reports/YYYY-MM-DD-coverage.md` SHALL contain:
...
The report SHALL be saved as `evals/discovery/reports/YYYY-MM-DD-coverage.md` using the date the eval was run. If a report for that date already exists, the eval prompt SHALL append a suffix (e.g., `-2`) rather than overwriting.
