...
Manifest filenames use object/action/role names that align with the governing OpenSpec capability without repeating the `act-` phase prefix, such as `entity-discovery.md`, `platform-comparison.md`, `platform-discovery-benchmark.md`, or `platform-discovery-report.md`.
...
For prompt review, save the generated resolved prompt snapshot under `observe/<action>-resolved-prompt-<resolver-short>.md` and review that snapshot against the manifest and required contracts. The saved snapshot is review evidence, not a replacement for the manifest.
